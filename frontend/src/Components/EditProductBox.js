...
export default function EditProductBox(props) {
 const {onClose,Name,Slug,ImageName,Price,StockCount,Description,Category,Brand,cat_slug}=props
 const init_slug=Slug;
  ...
  const submitHandler=async(e)=>{
    e.preventDefault();
      ...
      if(name){
        if(slug){
          try{
            const {data}= await axios.post(`/api/admin/editproduct/${cat_slug}/${init_slug}`,{
              name:name,
              slug:slug,
              imageName:imageName,
              price:price,
